...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("SELECT * FROM users WHERE email = ? AND role IN ('staff', 'admin')");
    $stmt->execute([$email]);
    $user = $stmt->fetch();
    ...
    if (!$user || !password_verify($password, $user['password_hash'])) {
        ...
        log_action($pdo, $user['user_id'], 'login', 'Staff/admin logged in');
...
<div class="row justify-content-center">
    <div class="col-md-5">
        <div class="card p-4">
            <h3 class="mb-4">Records Office Staff Login</h3>
